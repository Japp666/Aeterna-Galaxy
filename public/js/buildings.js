...
try {
    ...
    function initializeBuildings() {
        ...
        for (const [id, data] of Object.entries(buildingsData)) {
            ...
            const level = window.getPlayer().buildings[id]?.level || 0;
            card.innerHTML = `
    ...
    function refreshBuildingUI(buildingId) {
        ...
        if (card) {
            const level = window.getPlayer().buildings[buildingId]?.level || 0;
        ...
        document.querySelectorAll('.build-button').forEach(button => {
            ...
            button.onclick = async () => {
                if (canAfford) {
                    ...
                    const added = await window.addBuildingToQueue(id, data.buildTime);
                    if (added) {
