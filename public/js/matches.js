...
export function renderMatches() {
    ...
    <div class="matches-list">
      ${matches.map(match => `
        <div class="match-card">
          ...
          : (!match.result && match.home === gameState.club.name ? `<button id="simulate-match-${match.id}" class="button">Simulează meci</button>` : '')}
        </div>
      `).join('')}
    </div>
  `;
  matches.forEach(match => {
    if (!match.result && match.home === gameState.club.name) {
      ...
      button?.addEventListener('click', () => simulateMatch(match));
    }
  });
}

export function simulateMatch(match) {
  const matchObj = gameState.season.matches.find(m => m.id === match.id);
  if (!matchObj || matchObj.result) return;
  ...
  matchObj.result = { homeGoals, awayGoals };
  updateStandings(matchObj);
  updateClubStats(matchObj);
  gameState.gameDate = new Date(gameState.gameDate.getTime() + 7 * 24 * 60 * 60 * 1000);
  checkEndOfMatchDay();
  saveGame();
  renderMatches();
