...
export function renderPitch(container, formationName, mentalityName) {
  ...
  layout.forEach(slot => {
    ...
    // poziționare procentuală minus jumătate din dimensiune (8%/2)
 ...
 */
export function placePlayersInPitchSlots(
  container,      // element teren
  teamFormation,  // { pos: playerId, ... }
  availableEl,    // lista jucători disponibili
  allPlayers      // array de player objects
) {
  // curăță teren
  ...
  // actualizează lista de disponibile
  renderAvailablePlayers(availableEl, allPlayers);
}

/**
 * Randare jucători disponibili: doar cei fără onPitch===true
 */
export function renderAvailablePlayers(container, allPlayers) {
  container.innerHTML = '';
  const avail = allPlayers.filter(p => !p.onPitch);
  avail.forEach(p => {
